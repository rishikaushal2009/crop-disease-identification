...
```
.
├─ extract_pdf_text.py            # Utility used to extract PDF instructions
├─ extracted_take_home_task.txt   # Parsed task requirements
...
3. (Optional) Configure Kaggle API to auto-download dataset:
    ...
    - Test the CLI:
      ...
      - Download one PlantVillage variant (choose ONE):
         - Full raw (example slug, adjust if needed): `plantvillage/plantvillage-dataset`
      ...
      - After download, move or rename into `data/PlantVillage/<class folders>` as expected by the notebook.
      - Alternatively use helper script (handles download + optional reorg):

      
         ```powershell
         python download_dataset.py --slug emmarex/plantdisease --output data/raw
...
- Default: Downloads PlantVillage dataset via Kaggle API (`plantvillage-dataset` or similar) OR expects a local folder `data/PlantVillage`.
- You can alternatively use a smaller pre-split dataset; adjust the `DATA_ROOT` constant in the notebook.
...
- Size: Compact (~2–3M params). Easier OTA updates; lower bandwidth and storage.
